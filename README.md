...
```
python3 demo.py --checkpoint=data/pretrained_model/emo-body-lang_checkpoint.pt --img_file examples/COCO_val2014_000000019667.jpg
```
#### For video input:
```
# video with single person
python3 demo.py --checkpoint=data/pretrained_model/emo-body-lang_checkpoint.pt --vid_file examples/dancer.mp4
# video with multiple persons
python3 demo.py --checkpoint=data/pretrained_model/emo-body-lang_checkpoint.pt --vid_file examples/flashmob.mp4
```
...
```
python3 eval_coco.py --checkpoint=data/pretrained_model/emo-body-lang_checkpoint.pt
```
...
# Example usage:
# 3DPW
python3 eval.py --checkpoint=data/pretrained_model/emo-body-lang_checkpoint.pt --dataset=3dpw --log_freq=20
```
...
```
# training on COCO
CUDA_VISIBLE_DEVICES=0 python3 train.py --regressor emo_body_lang --single_dataset --misc TRAIN.BATCH_SIZE 64
# training on mixed datasets
CUDA_VISIBLE_DEVICES=0 python3 train.py --regressor emo_body_lang --pretrained_checkpoint path/to/checkpoint_file.pt --misc TRAIN.BATCH_SIZE 64
```
